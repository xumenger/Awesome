...
class UTC(datetime.tzinfo):
    def __init__(self, utc):
        ...
        if mt:
            ...
            raise ValuesError('bad utc time zone')
...
class Request(object):
    ...
    @property
    def path_info(self):
        return urllib.unquote(self._environ.get('PATH_IFNO', ''))
    ...
    def status(self, value):
        if isinstance(value, (int, long)):
            if value>=100 and value<=999:
                ...
                raise ValuesError('Bad response code: %s' % value)
...
class Jinja2TemplateEngine(TemplateEngine):
    ...
    def __call__(self, path, model):
        return self._env.get_tempalte(path).render(**model).encode('utf-8')
...
def _load_module(module_name):
    ...
    import_module = module_name[last_doct+1:]
    ...
    def get_wsgi_application(self, debug=False):
        ...
        def wsgi(env, start_response):
            ...
            try:
                ...
                stacks = fp.getvalue()
                fp.close()
                start_response('5000 Internal Server Error', [])
                return [
